...
<div class="orders-index">
    ...
    <script>
        function remove_orders(id, count, cart) {
            let form = new FormData();
            form.append('select_id', id);
            form.append('count_prod', count);
            form.append('cart_prod', cart);
            ...
            .then(result => {
                ...
                if (result.includes('false')){
                    ...
                    body.innerHTML = "<p>Товар возвращен в корзину</p>"
                }
    ...
    <?php
        ...
        if($products) {
            ...
            foreach (array_reverse($products) as $product) {
                ...
                echo "<p class='text-success'><b>Оплачено:</b> {$price} руб</b></p>
                    ...
                    if($product['status'] == 'Новый') {
                        ...
                        <button class='btn btn-danger text-white' onclick='remove_orders({$product['number_order']}, {$product['count']}, {$product['id_product']})'>Удалить</button>";
                    } 
            ...
            echo "<p class='mt-3'>Заказы отсутствуют</p>";
        }
    ?>
